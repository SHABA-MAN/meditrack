          ...
          <div>
            ...
            <div className="space-y-2">
              {studyItems.map((item, idx) => (
                <div key={idx} className="bg-blue-50 border border-blue-100 rounded-none p-3 text-sm">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-blue-900">
                      {item.subject} - Lec {item.number}
                    </span>
                    <span className="text-xs text-blue-600 bg-blue-100 px-2 py-0.5 rounded-none">
          ...
          <div>
            ...
            <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
              {taskItems.map((item, idx) => (
                <div key={idx} className="bg-emerald-50 border border-emerald-100 rounded-none p-3 text-sm">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.stage && (
                        <span className="text-xs text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded-none inline-block mt-1">
                          {item.stage}
                        </span>
                      )}
    ...
    <div className="min-h-screen bg-gray-100" dir="rtl">
      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-none shadow-sm border border-slate-200 p-6">
            ...
            {loading ? (
              <div className="flex items-center justify-center py-20">
                <div className="animate-spin rounded-none h-10 w-10 border-b-2 border-slate-900"></div>
              </div>
            ) : (
                ...
                {days.map((day, idx) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={!day}
                      className={`
                        aspect-square rounded-none text-sm font-medium transition-all relative
                        ${!day ? 'invisible' : ''}
                      ...
                      {hasAchievements && !selected && (
                        <div className="absolute bottom-1 left-1/2 transform -translate-x-1/2 flex gap-0.5">
                          {dayData.items.filter(i => i.type === 'study').length > 0 && (
                            <div className="w-1.5 h-1.5 rounded-none bg-blue-500"></div>
                          )}
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-center gap-6 text-xs">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-none bg-blue-500"></div>
                <span className="text-sm text-slate-600">مذاكرة</span>
              </div>
              <div className="flex items-center gap-2">
          ...
          {/* تفاصيل اليوم */}
          <div className="lg:col-span-1 bg-white rounded-none shadow-sm border border-slate-200 p-6">
